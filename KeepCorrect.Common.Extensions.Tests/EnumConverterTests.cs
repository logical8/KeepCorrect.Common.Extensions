...
namespace KeepCorrect.Common.Extensions.Tests
{
    public class EnumConverterTests
    {
        ...
        [TestCase(Enum1.Three, Enum2.THREE)]
        [TestCase(Enum1.Five, Enum2.Fix)]
        public void ToEnumByUInt_ByDefault_ReturnsExpectedResults(Enum1 enum1, Enum2 enum2)
        {
            var result  = enum1.ToEnumByUInt<Enum1, Enum2>();
            Assert.That(result, Is.EqualTo(enum2));
        }
        
        [TestCase(Enum1.Four, Enum2.Four)]
        public void ToEnumByUInt_WhenStringValuesAreNotTheSame_ReturnsNotSupportedException(Enum1 enum1, Enum2 enum2)
        {
            Assert.Throws<NotSupportedException>(() => enum1.ToEnumByUInt<Enum1, Enum2>());
        }
